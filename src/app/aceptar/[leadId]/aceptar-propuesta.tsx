...
function StatusView({accent, icon, title, message}: StatusContent) {
  return (
    <div className={cardClass}>
      ...
      <h2 className="font-serif text-[32px] leading-none tracking-tight text-ink">{title}</h2>
      <p className="max-w-sm text-[14.5px] leading-relaxed text-muted">{message}</p>
    </div>
  );
}
...
function LoadingView() {
  return (
    <div className={cardClass}>
      <span className="text-[10px] tracking-[0.2em] text-faint uppercase">Cargando propuesta…</span>
      <div className="h-px w-24 animate-pulse bg-ochre" />
    </div>
  );
}
...
function ConfirmarView({
  ...
  return (
    <div className={cardClass}>
      <span className="text-[10px] tracking-[0.2em] text-ochre uppercase">
        Confirmá tu decisión
      </span>
      <h2 className="font-serif text-[32px] leading-none tracking-tight text-ink">
      ...
      <p className="max-w-sm text-[14.5px] leading-relaxed text-muted">
        ...
        {lead?.presupuesto != null ? (
          <>
            {" "}
            por{" "}
            <span className="font-serif text-[19px] text-ochre">
      ...
      <div className="flex w-full max-w-sm flex-col gap-3">
        ...
        <div className="flex gap-3">
          ...
          <button
            className="ease flex-1 px-4 py-3.5 text-[11px] tracking-[0.14em] text-mist uppercase transition duration-200 hover:text-brick disabled:cursor-not-allowed disabled:opacity-40"
            disabled={enviando}
            type="button"
            onClick={onRechazar}
          >
...
function PedirCambiosView({
  ...
  return (
    <div className={cardClass}>
      <span className="text-[10px] tracking-[0.2em] text-ochre uppercase">Pedir cambios</span>
      <div className="w-full max-w-sm">
        ...
        <textarea
          className="ease w-full resize-y border-b border-rule bg-transparent pt-1 pb-3 text-[15px] leading-relaxed text-ink placeholder-mist outline-none transition duration-200 hover:border-mist focus:border-ochre"
        ...
        <div className="mt-6 flex gap-3">
          <button
            className="ease bg-ink px-6 py-3.5 text-[11px] font-medium tracking-[0.18em] text-paper uppercase transition duration-200 hover:bg-ochre disabled:cursor-not-allowed disabled:opacity-40"
          ...
          <button
            className="ease px-4 py-3.5 text-[11px] tracking-[0.14em] text-mist uppercase transition duration-200 hover:text-ink disabled:opacity-40"
            disabled={enviando}
            type="button"
            onClick={onVolver}
          >
