...
- **Rate limiting real, verificado en vivo.** Lo implementado en 5.3.1 no se
  probó contra una instancia de n8n corriendo: falta confirmar que la consulta
  compile y que el 429 llegue al navegador. Y para `lead/nuevo` puntualmente,
  falta la reestructuración a `responseNode` si se quiere que también devuelva
  429 en vez de sólo cortar el procesamiento interno.
