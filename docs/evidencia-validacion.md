...
**Ejecución:** 2026-09-02T17:53:14.177Z  ·  **n8n:** `http://localhost:5678`

## Escenarios

| # | Tabla 9 | Escenario | Resultado | Tiempo |
|---|---|---|---|---|
| 1 | E1 | Un lead de presupuesto alto se califica HOT y queda esperando propuesta | OK | 5551 ms |
| 2 | E1b | El profesional fija precio, plazo y alcance, y recién ahí sale la propuesta | OK | 3490 ms |
| 3 | E2 | Un lead de valor medio se califica WARM y también espera propuesta | OK | 4279 ms |
| 4 | E3 | Un lead de presupuesto bajo se califica COLD y NO recibe propuesta | OK | 4823 ms |
| 5 | E4 | Un lead inválido se rechaza sin persistirse y queda registrado | OK | 5243 ms |
| 6 | — | La propuesta se lee sólo con el token correcto | OK | 725 ms |
| 7 | E5/E6 | Aceptación válida y reutilización del enlace: una sola factura | OK | 8758 ms |
| 8 | — | El pago simulado es idempotente | OK | 4637 ms |
| 9 | E8 | El rechazo de la propuesta deja el lead en PERDIDO | OK | 6034 ms |
| 10 | E9 | El pedido de cambios vuelve el lead a EN_SEGUIMIENTO y guarda el mensaje | OK | 5260 ms |
| 11 | E10 | El estado del trabajo se actualiza desde el panel y se sincroniza | OK | 762 ms |
| 12 | — | Un token vencido no permite aceptar la propuesta | OK | 7644 ms |
...
- ✅ la fecha de cobro no cambió al reusar el enlace: "2026-09-02T17:51:05.687605+00:00"
...
|---|---|---|---|
| lead-hot | alta del lead en la base | 2164 ms | POST /lead/nuevo → fila en leads (hot) |
| propuesta-enviar | propuesta enviada | 1538 ms | incluye el email de propuesta y la card de Notion |
| lead-warm | alta del lead en la base | 910 ms | POST /lead/nuevo → fila en leads (warm) |
| lead-cold | alta del lead en la base | 941 ms | POST /lead/nuevo → fila en leads (cold) |
| aceptacion-atomica | aceptación → factura emitida | 2593 ms | incluye la generación del PDF con Gotenberg y el envío por email |
| aceptacion-atomica | respuesta al navegador | 3328 ms | las dos peticiones de aceptación en paralelo |
| pago-idempotente | pago confirmado | 256 ms | marca la factura como COBRADO |
| rechazo | alta del lead en la base | 903 ms | POST /lead/nuevo → fila en leads (rechazo) |
| rechazo | rechazo procesado | 241 ms |  |
| pedido-cambios | alta del lead en la base | 889 ms | POST /lead/nuevo → fila en leads (cambios) |
| pedido-cambios | pedido de cambios procesado | 255 ms |  |
| estado-trabajo | cambio de estado de trabajo | 226 ms | incluye la sincronización con Notion |
| token-vencido | alta del lead en la base | 823 ms | POST /lead/nuevo → fila en leads (vencido) |

**Generación de la factura en PDF (Gotenberg), extremo a extremo:** 2593 ms.
